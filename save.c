...
{
	FILE *f; 
	GtkTextIter p;/* Iterator is an object that represents positions between two characters in a buffer */
	...
	while((l = fread(fbuf, 1, sizeof(fbuf), f)) > 0) 
	{
		...
		gsize br, bw;
		gchar *text;
		if(!(text = g_locale_to_utf8(fbuf, l, &br, &bw, &err))) /* only text files in Utf-8 format can be opened */
		{
...
static gboolean save_if_modified(void)
{
	...
	dialog = gtk_message_dialog_new(GTK_WINDOW(window), 0, GTK_MESSAGE_QUESTION,GTK_BUTTONS_NONE,"GoodBye,Please Save changes to document before closing");
